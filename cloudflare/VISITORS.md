...
- Web Analytics **未啟用**：現有 Wrangler OAuth 存取 RUM site-info API 回覆 403。使用者可到 Cloudflare Dashboard > Web Analytics > Add a site，填 wackyjazz.github.io，取得公開 beacon token 後加入三頁（不要把 API token 放前端）。本次沒有將未啟用的功能宣稱完成。
...
網站來源在根目錄 guide/，需同步 deploy/github-r2/web/ 後推送並手動執行 pages.yml。重建圖庫／歌詞 HTML 時保留 visitors.js script。
