(() => {
  ...
  function updateProgress() { $('#progress-label').textContent = `${progress.read.length} / ${events.length}`; $('#read-progress').max = events.length; $('#read-progress').value = progress.read.length; }
  ...
  function readHash(){if(/^#e275(?:\/frame\/\d+)?$/.test(location.hash))history.replaceState(null,'',location.hash.replace('#e275','#e274'));const match=/^#(e[0-9]+)(?:\/frame\/([0-9]+))?$/.exec(location.hash);if(match&&byId.has(match[1]))openEvent(match[1],{noHash:true,frame:match[2]?Number(match[2])-1:0});}
